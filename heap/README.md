...
문제 접근
>주식 가격이 담긴 배열에서 앞에서 하나씩 꺼내서 다음 주식가격들과 비교하며 해결한다.

>시간을 재서 만약 가격이 떨어지면 그 때까지의 시간을 저장하고 끝까지 가격이 떨어지지 않아도 그 때까지의 시간을 저장한다.

>일반적인 리스트가 요소에 접근해서 삽입 또는 제거를 할 경우, O(n)이 소요되는 반면에, deque는 O(1)로 접근 가능하기 때문에 stack을 표현할 때 deque를 사용해서 한다.
