...
# 구분을 위해 연결되지 않은 부분은 "?"", 이긴 쪽은 "WIN", 진 쪽은 "LOSE"로 구분했습니다.

# Floyd-Warshell 알고리즘의 기본적인 논리는 i점에서 j점을 갈 때 k 점을 거쳐 i -> k -> j로는 갈 수 있는가? 이다.
# i -> k 가 "LOSE"이고 k -> j 가 "LOSE"라면 이는 i가 k에게, k가 j에게 졌다는 것을 의미한다. 그렇다면 matrix[i][j]는 Lose가 된다. 또한 matrix[j][i]는 True가 된다. 