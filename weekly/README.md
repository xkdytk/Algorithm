...
출처 : https://programmers.co.kr/learn/courses/30/lessons/86048

문제 접근
> 퇴실 목록을 확인하면서 퇴실할 사람이 방에 없는 경우에 추가적으로 입실시키면서 입실할 때 마다 만난 사람을 갱신해준다.

> 퇴실할 사람이 방에 있는 경우에는 그 사람을 현재 방에 있는 사람 리스트에서 제거한다.

> 입실을 추가로 할 때마다 지금까지 있던 사람들의 만난 사람 횟수는 1 증가시켜주고 지금 들어오는 사람들은 기존에 방에 있던 사람 수 만큼 추가해준다.